...
interface CardDao {
    ...
    @Query("SELECT * FROM Card WHERE tradingCardGame=:tradingCardGame AND idSet=:idSet AND supertype IN (SELECT name FROM CardType WHERE name=supertype) ORDER BY number")
